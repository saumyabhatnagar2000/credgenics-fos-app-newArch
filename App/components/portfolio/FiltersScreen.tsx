  ...
  const handleSearch = (text: string) => {
    ...
    const filteredData = statuses.filter(status => {
      if (activeFilterType == PortfolioFilterType.status)
        return String(status.text).toLowerCase().includes(formattedQuery);
              ...
              <FlatList
                ...
                refreshControl={
                  <RefreshControl
                    ...
                    refreshing={loading}
                    colors={[BLUE_DARK]}
                    onRefresh={() => getFilterList(activeFilterType)}
                  />
                }
                renderItem={status => {
                  let present: any =
                    activeFilterType == PortfolioFilterType.status
                      ? selected[status.item.text]
                      : activeFilterType == PortfolioFilterType.nbfc
                  ...
                  const onCheckboxClicked = () => {
                    ...
                    if (activeFilterType == PortfolioFilterType.status) {
                      ...
                      if (!present) {
                        ...
                        delete selected[status.item.text];
                      ...
                      if (!present) {
                        ...
                        delete nbfcFiltersSelected[status.item];
                        setNBFCFiltersSelected({
                          ...nbfcFiltersSelected,
                        });
                        select = false;
                      ...
                      if (!present) {
                        ...
                        delete tagsFilterSelected[status.item];
                        setTagsfilterSelected({
                          ...tagsFilterSelected,
                        });
                        select = false;
