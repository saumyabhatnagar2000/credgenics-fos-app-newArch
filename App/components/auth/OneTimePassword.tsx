...
export default function OneTimePassword({style, otpType, password, username}) {
    ...
    try {
      ...
      if (apiResponse) {
        if (otpType == 'login') {
          ...
          try {
            const response = await getUserDetails(output?.authentication_token);

            if (response) {
              const outputData = response?.data?.data?.user_details;
              let userData = {
                authenticationtoken: output?.authentication_token,
          ...
          ToastAndroid.show(apiResponse?.data?.message, ToastAndroid.SHORT);
          setTimeout(
            () =>
  ...
  const onError = (text: string | undefined) => {
    setShowError(text);
    if (text && text.length) setTimeout(() => setShowError(undefined), 3000);
  };
  ...
  return (
    <View style={style}>
      ...
      <Typography variant={TypographyVariants.heading1} style={styles.header}>
        Verification Code
      </Typography>
      <Typography
