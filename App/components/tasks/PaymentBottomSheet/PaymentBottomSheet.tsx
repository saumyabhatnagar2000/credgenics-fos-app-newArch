...
const PaymentLinkBottomSheet = React.forwardRef((props: any, ref) => {
  ...
  useEffect(() => {
    ...
    if (collection_modes.includes(CollectionModeTypes.payment_link_qr)) {
      tempCollectionArr.push(PAYMENT_TYPES.qrLink, PAYMENT_TYPES.paymentLink);
    }
  ...
  const onBankTransferClicked = () => {
    if (onlineCollectionMode.split(',').length == 1) setOnlineTabChangeIdx(0);
  ...
  const onSendClicked = async () => {
    ...
    try {
      const otpValue =
        otpType == OTP_TYPES.email ? OTP_TYPES_KEYS.email : OTP_TYPES_KEYS.sms;
    ...
    switch (stepNumber) {
      case PaymentSheetStates.sendLink:
        return (
          <View>
            ...
            />
            <View>
              {tabs && tabs?.map(tab => (tab.active ? tab.screen : null))}
            </View>
          </View>
        );
      ...
      case PaymentSheetStates.linkSent:
        ...
        if (Object.keys(selected).length == 0) {
          commAddresses = otpType == OTP_TYPES.email ? emailIds : mobileNumbers;
        ...
        return (
          <View style={styles.container}>
            ...
            {loading ? (
              ...
              <>
                <View style={styles.linkSentContainer}>
                  <AntDesign name="close" color={CANCEL_RED} size={20} />
                  <Typography
                    style={[styles.linkSentText, styles.errorText]}
                    variant={TypographyVariants.body3}>
                ...
                (!isPaymentTemplatePresent.sms ||
                  ...
                  <View
                    ...
                    <View style={{flexDirection: 'row'}}>
                      ...
                      <TouchableOpacity onPress={onTryOtherModeClick}>
                        <Typography
                          style={styles.clickableText}
                          variant={TypographyVariants.body3}>
                          Try other mode.
  ...
  const GenerateQRScreen = () => {
    return !showQrCode ? (
      <View
        ...
        <ImageBackground source={BLUR_QR} style={{width: 250, height: 250}} />
        <LinearGradientHOC
    ...
    return (
      <View style={styles.container}>
        ...
        <View
          ...
          <BottomSheetFlatList
            data={otpType == OTP_TYPES.email ? emailIds : mobileNumbers}
        ...
        {loading ? (
          ...
          <View
            ...
            <View style={{marginLeft: 10}}>
              <CustomButton
                style={[styles.buttonStyle, {marginTop: RFPercentage(3)}]}
                variant={TypographyVariants.body3}
  ...
  const handleTabChange = (label: string) => {
    ...
    tabDummy.map((tab, idx) => {
      ...
      tab.active = false;
      if (tab.label === label) {
        if (tab.label == tabs[0].label) setPaymentType(PAYMENT_TYPES.qrLink);
