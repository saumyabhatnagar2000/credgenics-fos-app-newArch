...
import {BLUE_DARK, LINEAR_GRADIENT_2} from '../../constants/Colors';
import Typography, {
...
export default function CustomAppBar(config: AppBarType) {
  ...
  const {allocationMonth, setAllocationMonth, setIsRightDrawer, companyType} =
    useAuth();
  ...
  return (
    <>
      <View>
        <StatusBar style={inverted ? 'dark' : 'light'} translucent />
        <NewMonthPicker
        ...
        <LinearGradient
          ...
          <Appbar.Header
            style={[
              styles.top,
              {
                backgroundColor: inverted ? '#fff' : 'transparent',
                elevation: 0,
                shadowOffset: {
                  width: 0,
                  height: 0, // for iOS
            ...
            {config.backButton && (
              <TouchableOpacity
                ...
                onPress={config?.onBackClicked ?? navigation.goBack}>
                <ChevronLeft
            ...
            {config.notifications && <Appbar.Action icon="bell-outline" />}
            {config.filter && <Appbar.Action icon="filter-outline" />}
            {config.add && (
              <Appbar.Action
            ...
            {config.options && (
              <Menu
                ...
                <Menu.Item
                  ...
                  title={moment(allocationMonth, 'YYYY-M-DD').format(
                    'MM, YYYY',
                  )}
                  icon="calendar"
                />
                <Divider />
                <Menu.Item onPress={() => {}} title="Sort" icon="sort" />
                <Menu.Item onPress={() => {}} title="Filter" icon="filter" />
              </Menu>
            )}
