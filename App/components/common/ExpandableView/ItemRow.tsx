...
export const ItemRow = ({
  ...
  useEffect(() => {
    ...
    if (
      (filterDetails?.visit_status?.toLowerCase() === TaskStatusTypes.open ||
      ...
      if (
        ...
        const handleSendReceipt = async () => {
          try {
            const response = await sendReceipt(
              extraData.loan_id,
              dataDict.id,
              extraData.visit_date ?? extraData.loan_data.visit_date,
        ...
        return (
          <TouchableOpacity
            ...
            style={[styles.rowContainer, {marginLeft: RFPercentage(1)}]}>
            <SendIcon color={getPDFButtonColor(extraData.visit_status)} />
            <Typography
              variant={TypographyVariants.body3}
              style={{
                marginLeft: 3,
  ...
  const valueModifier = (key: string, data?: any) => {
    ...
    if (data) {
      if (typeof data === 'object') {
        ...
        } else if (key === 'call_start_time' || key === 'call_end_time') {
