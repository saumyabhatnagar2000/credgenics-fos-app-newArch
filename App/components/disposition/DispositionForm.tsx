...
export const DispostionForm = ({
    ...
    try {
      ...
      if (apiResponse) {
        if (apiResponse?.data?.subdispositions.length > 0) {
          ...
          apiResponse.data.subdispositions.map((item: DispositionType) => {
  ...
  const handleSubDispositionOneChange = async (id: string) => {
    setErrorsObject({});
    setSelectedDispositionOne(getDipositionObject(id, subDispositionOneList));
    const tempSelected: DispositionType = getDipositionObject(
      id,
    ...
    if (!amountRequired)
      setAmountRequired(tempSelected.committed_amount_required);
    if (!reminderRequired) setReminderRequired(tempSelected.reminder_required);
  ...
  const handleSubdispositionTwoChange = (id: string) => {
    setErrorsObject({});
    setSelectedDispositionTwo(getDipositionObject(id, subDispositionTwoList));
    const tempSelected: DispositionType = getDipositionObject(
      id,
  ...
  const getLoanData = async () => {
    try {
      ...
      if (error.response) {
        message = error?.response?.data.output ?? error?.response?.data.message;
      }
    ...
    try {
      ...
      if (apiRepsonse) {
        if (dispositionID.length == 0) {
          ...
          let dummyDisposition: Array<DropDownType> = [];
          apiRepsonse.data?.dispositions.map((item: DispositionType) => {
            dummyDisposition.push({
              label: item.text,
              value: item.id,
            });
  ...
  const onSubmit = async () => {
    ...
    if (reminderDate.length > 0) {
      ...
      if (checkedStep === 'Call') {
        ...
        if (isPTP) {
          try {
            const [newReminderRes, submitDispositionRes] = await Promise.all([
              createNewReminder(
                newReminderBody,
                loanId,
                allocationMonth,
            ...
            if (newReminderRes?.data || submitDispositionRes?.success) {
              ...
              try {
                ...
                if (newTaskResponse?.success) {
                  ToastAndroid.show(
                    newTaskResponse?.message ?? 'Visit/PTP created',
                    ToastAndroid.SHORT,
                  );
                }
              } catch (e: any) {}
            ...
            ToastAndroid.show(
              e.response?.data?.output ?? e.response?.data?.message ?? 'Error',
              ToastAndroid.SHORT,
            );
          }
          navigateToHome();
  ...
  return (
    <>
      <ClosedLoanErrorModal visible={showAlertDialog} navigation={navigation} />
        ...
        <View style={{flexDirection: 'column'}}>
          <View style={styles.formRowContainer}>
            <View style={styles.formLabelContainer}>
              <Typography style={styles.formLabel}>Contact Number:</Typography>
            </View>
            <View style={styles.inputBox}>
              <TextInput
                value={phoneNumber ?? ''}
                editable={false}
                style={[styles.inputContainer, {color: '#043E90'}]}
                textAlign="left"
              />
            </View>
          </View>
          <View style={[styles.formRowContainer]}>
            <View style={styles.formLabelContainer}>
              <Typography style={styles.formLabel}>Disposition*:</Typography>
            </View>
            <View style={[styles.inputBox]}>
              <View style={{width: RFPercentage(17)}}>
                <CustomDropDown
                  ...
                  onChangeValue={(value: string) => {
                    if (value.length > 0) handleDispositionChange(value);
                  }}
                  containerStyle={{
                    paddingHorizontal: 0,
                  }}
            ...
            <View
              ...
              <View style={styles.formLabelContainer}>
                <Typography style={styles.formLabel}>
                  {`Sub Disposition 1${subDispositionOneReq ? '*' : ''}:`}
                </Typography>
              </View>
              <View style={[styles.inputBox]}>
                <View style={{width: RFPercentage(17)}}>
                  <CustomDropDown
                    ...
                    zIndexInverse={2000}
                  />
                  {errorObject?.sub_disposition1?.length > 0 ? (
                    <Typography
                      style={styles.error}
                      variant={TypographyVariants.caption2}>
          ...
          {subDispositionTwoDropdown.length > 0 ? (
            <View
              ...
              <View style={styles.formLabelContainer}>
                <Typography style={styles.formLabel}>
                  {` Sub Disposition 2${subDispositionTwoReq ? '*' : ''}:`}
                </Typography>
              </View>
              <View style={[styles.inputBox]}>
                <View style={{width: RFPercentage(17)}}>
                  <CustomDropDown
                    ...
                    zIndexInverse={3000}
                  />
                  {errorObject?.sub_disposition2?.length > 0 ? (
                    <Typography
                      style={styles.error}
                      variant={TypographyVariants.caption2}>
