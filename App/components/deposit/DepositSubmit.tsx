  ...
  const enableSubmitButton = useCallback(() => {
    ...
    const branchRepresentativeCheck = () => {
      if (depositBranch?.branch_type == BankBranchType.company)
        return (
          !depositOtpVerificationMethod || branchRepresentativeId.length > 0
        );
      return true;
    };

    const proofCheck = () => {
      if (depositBranch?.branch_type == BankBranchType.airtel) return true;
        ...
        <View>
          ...
          <View style={styles.dropdownStyle}>
            ...
            {!!depositLocationList?.length &&
              depositLocationList?.map((branchList: object, index: number) => {
                return (
                  <ExpandableCardHelpSection
                    dataList={branchList}
                    key={index}
                    type={ExpandableCardTypes.depositLocation}
                    isOpened={false}
                    extraData={extraData}
                  />
                );
          ...
          <View style={styles.depositMethodContainer}>
            <Typography style={styles.labelStyle}>Deposit Mode</Typography>
            <CustomDropDown
              listMode="SCROLLVIEW"
