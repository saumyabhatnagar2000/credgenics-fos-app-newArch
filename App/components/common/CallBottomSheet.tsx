...
export const CallBottomSheet = React.forwardRef((props: any, ref) => {
  ...
  const updateReduxStorage = () => {
    ...
    if (applicantType == ApplicantTypes.applicant) {
      ...
      let tempLoanData = tempLoanDetailMap[selectedLoanData.loan_id].loan_data;
      tempLoanData.co_applicant[applicantIndex].co_applicant_contact_number =
  ...
  const onClickAdd = async () => {
    ...
    if (companyType == CompanyType.credit_line && transactionId?.length == 0) {
      ToastAndroid.show(
    ...
    try {
      ...
      if (companyType == CompanyType.loan) {
        ...
        const apiResponse = await updateContactDetails(loanId, data, authData);
        if (apiResponse?.data) {
          updateReduxStorage();
          setnewContactDetail('');
      ...
      if (e?.response?.data?.data?.error == 'Module access not permitted') {
      ...
      <>
        ...
        {!!item.length && (
          <TouchableOpacity
            ...
            <Typography variant={TypographyVariants.body3}>{item}</Typography>
            ...
            <View style={{paddingHorizontal: RFPercentage(2)}}>
              ...
              {contactNumber && contactNumber?.length && data ? (
                contactNumber?.map((item, index) => {
                  return (
                    <ContactNumberRow key={item} item={item} index={index} />
                  );
                })
              ) : (
                <Typography
