  ...
  return (
    <View style={styles.historyRow}>
      ...
      {heading == 'Deposit Status' ? (
        ...
        <Typography style={{flex: 3}} variant={TypographyVariants.body2}>
  ...
  const getHistory = async (linked_deposit_id: string) => {
    if (historyAvailable) {
      if (history.length == 0) {
        setLoading(true);
        try {
          const data = await loadDepostHistory(linked_deposit_id, authData);
      ...
      <TouchableOpacity
        ...
        <View style={styles.container}>
          <View style={styles.left}>
            <View>
              <View style={styles.row}>
                <DepositsImages type={data.deposit_method} size={16} />
                <CurrencyTypography
              ...
              <View style={[styles.row, {marginVertical: 2}]}>
                <Typography
                  ...
                  ID: {data.deposit_id} | {getOnlyDate(data.created)}
                </Typography>
              </View>
            </View>
            <View style={[styles.row, {paddingTop: 3, marginTop: 1}]}>
              <View style={[styles.row, {marginRight: 25}]}>
                ...
                <Typography variant={TypographyVariants.caption5}>
          ...
          <View style={styles.right}>
            <View style={styles.detailsContainer}>
              ...
              {data.verification_status == DepositStatuses.rejected ? (
      ...
      {showHistory && (
        <View style={styles.historyListContainer}>
          {loading ? (
            ...
            history?.map(
              (historyItem: DepositHistoryItemType, index: number) => (
